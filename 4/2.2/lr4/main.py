...
def save(filename):
    ...
    for company_name, company_info in companies.items():
        ...
        f.write(str(company_info["vehicles"]) + '\n')
        f.write(str(company_info["cost_per_km"]) + '\n')
        f.write(company_info["address"] + '\n')
        f.write(str(company_info["max_weight"]) + '\n')
    f.close()

def load(filename):
    with open(filename, "r") as f:
        while True:
            company_name = f.readline().strip() # читаємо рядок з видаленням всього непотрібного в тому числі і переносів
            if not company_name:  # Якщо прочитали порожній рядок, то вже кінець файлу
                break
            vehicles = int(f.readline().strip())
            cost_per_km = float(f.readline().strip())
